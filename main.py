# main.py - FINAL FFmpeg ENABLED VERSION
...
class WorkingVideoDownloader:
    ...
    def get_video_info(self, url):
        """Get video information with proper formats, including merged option"""
        try:
            ...
            with yt_dlp.YoutubeDL(self.ydl_opts) as ydl:
                ...
                if not thumbnail and 'thumbnails' in info:
                    ...
                    if thumbnails:
                        thumbnail = thumbnails[-1].get('url', '')  # Last thumbnail is usually best
                ...
                if 'formats' in info:
                    for fmt in info['formats']:
                        # Skip formats that are only headers or unplayable
                ...
                # --- ADD Merged Format Option (For FFmpeg) ---
                ...
                # Add a merged audio option (For FFmpeg)
    ...
    def download_video(self, url, format_id, quality):
        ...
        try:
            ...
            if format_id == 'bestaudio':
                ...
                # --- Merged Video+Audio Logic (Requires FFmpeg) ---
                download_opts = {
                    'outtmpl': filepath_no_ext + '.%(ext)s', 
                    # This format string forces yt-dlp to use FFmpeg to merge best video and best audio
                    'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best',
                    'merge_output_format': 'mp4',
                }
                
            else:
                # --- Specific Format Download Logic ---
                download_opts = {
                    'outtmpl': filepath_no_ext + '.%(ext)s',
            ...
            with yt_dlp.YoutubeDL(download_opts) as ydl:
                ...
                # --- CRUCIAL: Find the actual downloaded file path ---
                files = os.listdir(temp_dir)
                for f in files:
                    # Look for the file that starts with our unique base filename
            ...
            if not final_filepath or not os.path.exists(final_filepath):
                raise Exception("Download failed - final file not found. Merging might have failed.")
...
def download_video_route():
    ...
    try:
        ...
        url = data.get('url', '').strip()
        # Default to the best merged format for guaranteed playback
        format_id = data.get('format_id', 'best_merged_mp4') 
        quality = data.get('quality', 'Best Quality (MP4 - Merged)') # Use default quality name
        ...
        # --- Cleanup hook: Delete the file after it's sent to the user ---
        @app.after_request
